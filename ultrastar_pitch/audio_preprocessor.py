...
class AudioPreprocessor:
    ...
    def __init__(self, sr=16000, win_len=4096, stride=1024):
        """ init state variables """
        err_str1 = "{0} has to be an positive integer value"
        assert isinstance(sr, int) and sr > 0, err_str1.format("sample_rate")
        assert isinstance(win_len, int) and win_len > 0, err_str1.format("win_len")
    ...
    def transform(self, segment):
        ...
        if len(segment) < self.win_len:
            ...
            mdct = abs(dct([segment], type=4, n=self.win_len, axis=1)[:,:self.win_len // 2])
        else:
